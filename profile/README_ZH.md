...
```
Prompt/需求 —→ 蓝图/意图 —→ 逻辑链/架构 —→ 逻辑块 —→ 代码块/cpp —→ 代码/asm
              ↑ 人类可读    人类可读     人类可读     可审计       廉价产物
              └──────── 人类输入层 ────────────────────┘
                              ↓
                         AI 是"编译器"
                              ↓
...
│                           人类编辑层（源数据）                             │
├─────────────────────────────────────────────────────────────────────────┤
│  L5: Blueprint    │  意图文档（why）                                      │
│  L4: Logic Chain  │  程序框图/架构图（what）                               │
│  L3: Logic Block  │  伪代码/契约（how）                                   │
├─────────────────────────────────────────────────────────────────────────┤
│                         SVP 编译管道（单向）                               │
├─────────────────────────────────────────────────────────────────────────┤
│  L2: Code Block   │  AI 生成的代码模板（impl skeleton）                    │
│  L1: Code         │  实际可运行代码（full implementation）                 │
└─────────────────────────────────────────────────────────────────────────┘

理念：

1. 上层是源数据，下层是派生产物 —— 就像 .c → .o → binary
2. 人类只编辑 L5-L3，AI 负责编译到 L2-L1
3. 想改代码？改上层 IR，重新编译 —— 不允许直接修改 L1
